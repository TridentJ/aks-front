  ...
  function toDetail(
    ...
    if (model === "render") {
      ...
      useMultiTagsStoreHook().handleTags("push", {
        ...
        query: parameter,
        meta: {
          title: `客户详情 - ${parameter.customerName}`,
      ...
      // 路由跳转
      router.push({ name: "CustomerRender", query: parameter });
    } else if (model === "edit") {
      useMultiTagsStoreHook().handleTags("push", {
        path: `/customer/edit`,
        name: "CustomerEdit",
        query: parameter,
        meta: {
          title: `修改客户 - ${parameter.customerName}`
          // 如果使用的是非国际化精简版title可以像下面这么写
          // title: `No.${index} - 详情信息`,
        }
      });
      router.push({ name: "CustomerEdit", query: parameter });
    }
  }

  // 用于页面刷新，重新获取浏览器地址栏参数并保存到标签页
  const initToDetail = (model: "render" | "edit") => {
