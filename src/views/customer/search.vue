...
import {
  CustomerBaseSearchInfo,
  InvoiceSearchInfo,
  CustomerContactSearchInfo,
  InvoiceSearchResult,
  CustomerContactSearchResult
} from "@/interface/CustomerSearchInterface";
import { http } from "@/utils/http";
import { AjaxResponseList } from "@/api/AjaxResponseList";
import { ElMessage } from "element-plus";
import { useDetail } from "@/views/customer/customerRouter";
...
const pagChange = () => {
  ...
  if (searchState.value == 1) {
    ...
    formCustomerContacts.value.pageNum = currentPage.value;
...
const showDetails = (customerId: string, customerName: string) => {
...
const customerContactsSearchInitInfo = () => {
  return {
    id: 0,
    customerId: 0,
    customerName: "",
...
const formBase = ref<CustomerBaseSearchInfo>(customerSearchInitInfo());
const formInvoice = ref<InvoiceSearchInfo>(invoiceSearchInitInfo());
const formCustomerContacts = ref<CustomerContactSearchInfo>(
  customerContactsSearchInitInfo()
);

const customerBaseCreateTime = ref();
const customerBaseUpdateTime = ref();
const invoiceCreateTime = ref();
const invoiceUpdateTime = ref();
const customerContactsCreateTime = ref();
const customerContactsUpdateTime = ref();

const customerBaseSearchResult = ref<CustomerBaseInfo[]>([]);
const invoiceSearchResult = ref<InvoiceSearchResult[]>([]);
const customerContactsSearchResult = ref<CustomerContactSearchResult[]>([]);

//1：搜索基础信息 2：搜索发票 3：搜索联系人
const searchState = ref<Number>(1);

const searchCustomer = () => {
  //console.log(customerBaseCreateTime.value);
  if (customerBaseCreateTime.value != undefined) {
    formBase.value.createTimeStart = customerBaseCreateTime.value[0];
    formBase.value.createTimeEnd = customerBaseCreateTime.value[1];
  }
  if (customerBaseUpdateTime.value != undefined) {
    formBase.value.updateTimeStart = customerBaseUpdateTime.value[0];
    formBase.value.updateTimeEnd = customerBaseUpdateTime.value[1];
  }
  http
    .request<AjaxResponseList>("post", "/api/customer/search", {
      data: formBase.value
    })
    .then(function (response) {
      if (response.code == 0) {
        //成功
        customerBaseSearchResult.value = response.data;
        pagShow.value = true;
        ...
        //console.log(customerBaseSearchResult.value);
      } else {
        customerBaseSearchResult.value.length = 0;
...
const searchCustomerContacts = () => {
  searchState.value = 3;
  if (customerContactsCreateTime.value != undefined) {
    formCustomerContacts.value.createTimeStart = customerContactsCreateTime.value[0];
    formCustomerContacts.value.createTimeEnd = customerContactsCreateTime.value[1];
  }
  if (customerContactsUpdateTime.value != undefined) {
    formCustomerContacts.value.updateTimeStart = customerContactsUpdateTime.value[0];
    formCustomerContacts.value.updateTimeEnd = customerContactsUpdateTime.value[1];
  }
  http
    .request<AjaxResponseList>("post", "/api/customerContacts/search", {
      data: formCustomerContacts.value
    })
    .then(function (response) {
      if (response.code == 0) {
        //成功
        customerContactsSearchResult.value = response.data;
        pagShow.value = true;
        totalNum.value = response.total;
        currentPageSize.value = response.pageSize;
        currentPage.value = response.pageNum;
        ...
        customerContactsSearchResult.value.length = 0;
        ElMessage.warning(response.message);
      }
    })
    .catch(function (error) {
    ...
    <el-card class="search-card">
      <template #header>
        <div class="card-header">
          <span class="font-bold">搜索客户</span>
        </div>
      </template>
        ...
        <el-collapse-item name="search-condition">
          ...
          <el-tabs type="border-card">
                ...
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-form-item label="创建时间">
                      <el-date-picker
                        v-model="customerBaseCreateTime"
                        type="daterange"
                        unlink-panels
                        range-separator="到"
                  ...
                  <el-col :span="8">
                    <el-form-item label="更新时间">
                      <el-date-picker
                        v-model="customerBaseUpdateTime"
                        type="daterange"
                        unlink-panels
                        range-separator="到"
                        start-placeholder="开始时间"
            ...
            <el-tab-pane>
              ...
              <el-form
                :model="formCustomerContacts"
                label-width="100px"
                ref="customerContactsFormRef"
              >
                <el-row>
                  <el-col :span="8">
                    <el-form-item label="姓名">
                      <el-input
                        v-model="formCustomerContacts.ccName"
                        placeholder="姓名"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="部门">
                      <el-input
                        v-model="formCustomerContacts.department"
                        placeholder="部门"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="职位">
                      <el-input
                        v-model="formCustomerContacts.job"
                        placeholder="职位"
                      />
                    </el-form-item>
                  </el-col>
                ...
                <el-row>
                  <el-col :span="8">
                    <el-form-item label="工作地">
                      <el-input
                        v-model="formCustomerContacts.baseLocation"
                        placeholder="工作地"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="固定电话">
                      <el-input
                        v-model="formCustomerContacts.telephone"
                        placeholder="固定电话"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="手机">
                      <el-input
                        v-model="formCustomerContacts.phone"
                        placeholder="手机"
                      />
                    </el-form-item>
                  </el-col>
                ...
                <el-row>
                  <el-col :span="8">
                    <el-form-item label="邮箱">
                      <el-input
                        v-model="formCustomerContacts.mail"
                        placeholder="邮箱"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="QQ">
                      <el-input
                        v-model="formCustomerContacts.qq"
                        placeholder="QQ"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="微信">
                      <el-input
                        v-model="formCustomerContacts.wechat"
                        placeholder="QQ"
                      />
                    </el-form-item>
                  </el-col>
                ...
                <el-row>
                  <el-col :span="8">
                    <el-form-item label="状态">
                      <el-select
                        v-model="formCustomerContacts.state"
                        placeholder="状态"
                        style="width: 100%"
                      >
                        <el-option
                  ...
                  <el-col :span="8">
                    <el-form-item label="创建时间">
                      <el-date-picker
                        v-model="customerContactsCreateTime"
                        type="daterange"
                        unlink-panels
                        range-separator="到"
                        start-placeholder="开始时间"
                  ...
                  <el-col :span="8">
                    <el-form-item label="更新时间">
                      <el-date-picker
                        v-model="customerContactsUpdateTime"
                        type="daterange"
                        unlink-panels
                        range-separator="到"
                        start-placeholder="开始时间"
                ...
                <el-row>
                  <el-col :span="4" :offset="10">
                    <el-button type="primary" @click="searchCustomerContacts">
                      <IconifyIconOffline :icon="SearchLine" />&nbsp;&nbsp;搜索
                    </el-button>
                  </el-col>
                </el-row>
    ...
    <el-card v-show="searchResultCard">
      ...
      <div>
        <template v-if="searchState === 1">
          <template v-for="(item, index) in customerBaseSearchResult">
            <el-row>
              <el-col :span="24">
                <el-button
                  ...
                  size="large"
                  type="success"
                  @click="showDetails(item.id, item.customerName)"
                  >{{ item.customerName }}</el-button
                >
              </el-col>
              <br />
              <el-col :span="24">
            ...
            <el-row>
              <el-col :span="24">
                ...
                <el-button
                  ...
                  size="large"
                  type="success"
                  @click="showDetails(item.customerId, item.customerName)"
                  >{{ item.customerName }}</el-button
                >
              </el-col>
              <br />
        ...
        <template v-else-if="searchState === 3">
          <template v-for="(item, index) in customerContactsSearchResult">
            <el-row>
              <el-col :span="24">
                <el-button style="margin-bottom: 5px" type="warning" size="large" disabled>{{item.ccName }}</el-button>
                <el-button
                  style="margin-bottom: 5px"
                  size="large"
                  type="success"
